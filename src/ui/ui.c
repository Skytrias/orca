...
void oc_ui_layout_downward_dependent_size(oc_ui_context* ui, oc_ui_box* box, int axis)
{
    ...
    box->spacing[axis] = oc_max(0, count - 1) * box->style.spacing;
    ...
    switch(box->style.size.c[axis].kind)
    {
        ...
        case OC_UI_SIZE_PARENT_MINUS_PIXELS:
        {
            ...
            {
                box->minSize[axis] = minSum + box->spacing[axis] + 2 * box->style.margin.c[axis];
            }
        }
        break;
    }
    ...
    if(size->kind == OC_UI_SIZE_CHILDREN)
    {
        ...
        f32 margin = box->style.margin.c[axis];
...
void oc_ui_layout_upward_dependent_size(oc_ui_context* ui, oc_ui_box* box, int axis)
{
    ...
    if(!(box->flags & overflowFlag))
    {
        if(box->layoutAxis == axis)
        {
            ...
            while(1)
            {
                ...
                f32 totalContents = sum + box->spacing[axis] + 2 * box->style.margin.c[axis];
        ...
        else
        {
            ...
            oc_list_for(box->children, child, oc_ui_box, listElt)
            {
                if(!oc_ui_box_hidden(child) && !child->style.floating.c[axis])
                {
                    f32 totalContents = child->rect.c[2 + axis] + 2 * box->style.margin.c[axis];
                    f32 excess = oc_clamp_low(totalContents - box->rect.c[2 + axis], 0);
                    f32 relax = child->style.size.c[axis].relax;
                    f32 minSize = child->minSize[axis];
    ...
    {
        f32 minSize = sum + 2 * box->style.margin.c[axis] + box->spacing[axis];
...
void oc_ui_layout_compute_rect(oc_ui_context* ui, oc_ui_box* box, oc_vec2 pos)
{
    ...
    oc_ui_axis layoutAxis = box->layoutAxis;
    oc_ui_axis secondAxis = (layoutAxis == OC_UI_AXIS_X) ? OC_UI_AXIS_Y : OC_UI_AXIS_X;
    f32 spacing = box->style.spacing;

    oc_ui_align* align = box->layoutAlign.c;

    oc_vec2 origin = { box->rect.x,
                       box->rect.y };
    oc_vec2 currentPos = origin;

    oc_vec2 margin = { box->style.margin.x,
                       box->style.margin.y };
    ...
    if(draw && (box->flags & OC_UI_FLAG_DRAW_TEXT))
    {
        ...
        switch(box->layoutAlign.x)
        {
            ...
            case OC_UI_ALIGN_END:
                x = box->rect.x + box->rect.w - style->margin.x - textBox.w;
                break;
        ...
        switch(box->layoutAlign.y)
        {
            ...
            case OC_UI_ALIGN_END:
                y = box->rect.y + box->rect.h - style->margin.y - textBox.h + textBox.y;
                break;
...
void oc_ui_begin_frame(oc_vec2 size, oc_ui_style* defaultStyle, oc_ui_style_mask defaultMask)
{
    ...
    ui->overlay->layoutAlign = (oc_ui_layout_align) { OC_UI_ALIGN_START, OC_UI_ALIGN_START };
...
oc_ui_sig oc_ui_button_str8(oc_str8 label)
{
    ...
    oc_ui_style defaultStyle = { .size.width = { OC_UI_SIZE_TEXT },
                                 ...
                                 .textAlign = { OC_UI_ALIGN_CENTER, OC_UI_ALIGN_CENTER },
                                 .margin.x = 12,
                                 .margin.y = 6,
                                 .color = theme->primary,
                                 .bgColor = theme->fill0,
...
//         {
//             oc_ui_style frameStyle = layoutAxis = trackAxis,
...
//         }
//         f32 buttonWidth = maxOptionWidth + 2 * button->style.margin.x + button->rect.h;
...
//         f32 containerWidth = oc_max(maxOptionWidth + checkmarkSize + checkmarkSpacing + 2 * panel->style.margin.x,
//                                     button->rect.w);
...
//     oc_ui_style_next(&(oc_ui_stylelayoutAxis = OC_UI_AXIS_Y,
//                                      .spacing = 12 },
//                      OC_UI_STYLE_LAYOUT_AXIS
//                          | OC_UI_STYLE_LAYOUT_SPACING);
//     oc_ui_container_str8(name, 0)
...
//         for(int i = 0; i < info->optionCount; i++)
//         {
//             oc_ui_style_next(&(oc_ui_stylelayoutAxis = OC_UI_AXIS_X,
//                                              .spacing = 8,
//                                              .layout.align.y = OC_UI_ALIGN_CENTER },
//                              OC_UI_STYLE_LAYOUT_AXIS
//                                  | OC_UI_STYLE_LAYOUT_SPACING
...
//     oc_ui_style frameStyle = { .margin.x = 12,
//                                .margin.y = 6,
//                                .bgColor = theme->fill0,
//                                .roundness = theme->roundnessSmall };
